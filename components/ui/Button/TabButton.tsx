...
const TabButton: React.FC<TabButtonProps> = ({
    ...
    const height = type === "one" ? "h-[1.4375rem]" : type === "five" ? "h-12" : "h-9";
    ...
    const bgOne = "hover:bg-[#FFFFFF21] bg-transparent active:bg-[#2283f6] hover:shadow-[inset_0_0.0625rem_0_0_rgba(255,255,255,0.16)]";
    ...
    return (
        <div
            className={
                cn("flex group gap-2 text-casper active:text-white px-4 cursor-pointer hover:text-white  items-center justify-center rounded-[0.5rem]",
        ...
        >
            ...
            <span>{title}</span>
            {badge && (
                <span className={cn("px-2 py-0.5 rounded-[0.375rem] text-[0.75rem] font-bold", badgeStyle)}>{badge}</span>
            )}
        </div>
    );
};
