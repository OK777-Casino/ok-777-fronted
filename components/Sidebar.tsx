...
const Sidebar: React.FC = () => {
  ...
  const currentLanguageDisplay = sidebarData.languageData[currentLanguage.code as keyof typeof sidebarData.languageData] || sidebarData.languageData.zh;
  ...
  return (
    <div>
      <aside
        ref={sidebarRef}
        className={`sidebar bg-[#111923]/54 backdrop-blur-[2rem] fixed lg:block transition-all duration-300 z-40 overflow-y-auto overflow-x-visible h-[calc(100vh-9.9375rem)] lg:top-[3.5rem] top-[6.25rem] lg:h-[calc(100vh-3.5rem)] ${isCollapsed ? "close " : "open"
          }`}
        style={{
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <SidebarTopSection isCollapsed={isCollapsed} topButtons={sidebarData.topButtons} />

          {/* Main Content Sections */}
          <div className={`p-4 pt-0 ${isCollapsed ? "px-0" : ""} space-y-1 flex-1`}>
            <div className=" ">
              <SidebarSections
            ...
            <Link href={"/install-app"} className="py-2 justify-center lg:hidden flex flex-col gap-2   rounded-[0.5rem] h-[5.1875rem] px-4 bg-[linear-gradient(45deg,#111923,#003A81)]">
                <span className="text-[0.875rem] text-white font-bold">{t('app.title')}</span>
                <span className="text-casper text-[0.625rem]">{t('app.subtitle')}</span>
            </Link>
      ...
      <LanguageSelector
        ...
        onLanguageChange={(langCode) => setCurrentLanguage({ code: langCode, name: sidebarData.languageData[langCode as keyof typeof sidebarData.languageData]?.name || t('languages.chinese') })}
        initialLanguage={currentLanguage.code}
      />

      <ModalContainer isOpen={serviceModal} width="95vw" onClose={toggleServiceModal} title={t('help.onlineService')} className="!w-[46.25rem]">
        <div className="h-[14.875rem] w-[calc(100%+1.875rem)] overflow-hidden -mt-[0.9375rem] -ml-[0.9375rem] relative">
          <img src="/images/block-coin.png" alt="blockcoin" className="object-cover absolute grayscale z-[1] w-full h-full" />
          <div className="bg-[linear-gradient(#2283F6,#2283F600)] w-full h-full absolute top-0 left-0 z-[2]"></div>
          <img src="/images/calf/mascot14.png" className="absolute z-[3] w-[18.75rem] top-[20%] -right-[3.125rem] lg:right-[1.875rem]" alt="mascot" />
          <div className="relative z-[3] flex flex-col items-start justify-center pl-[1.875rem] lg:pl-[3.75rem] h-full gap-1">
            ...
            <span className="font-bold text-dodger-blue text-[1rem]">{t('help.onlineService')}</span>
            <span className="font-medium text-yellow-orange text-[0.875rem]">{t('help.hours24')}</span>
            <span className="font-medium text-white text-[0.875rem]">{t('help.dedicatedService')}</span>
        ...
        <div className="flex flex-col gap-2">
          {
            services.map((service, index) =>
              <div key={index} className="bg-white-4 rounded-[0.75rem] p-4 flex flex-col gap-2">
                <div className="flex justify-between items-center">
                  <div className="font-bold text-white text-[0.875rem] flex gap-2">
                    <div className="rounded-[0.5rem]  w-6 h-6 border flex justify-center items-center" style={{ borderColor: service.color }}>
                      {service.icon}
                    </div>
                    {service.title}
                  </div>
