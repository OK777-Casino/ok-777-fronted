...
const DataStatistics: React.FC = () => {
  ...
  const [selectedNetworkValue, setSelectedNetworkValue] = useState("Past 7 days");
    ...
    <div className=" [@media(max-width:660px)]:w-full">
      <div className="w-full flex flex-col gap-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 bg-white-4 rounded-[12px] overflow-hidden p-1 h-11">
          {
            ["Game data", "Betting data"].map((item, index) => <div className={cn("text-[14px] font-bold rounded-[8px] overflow-hidden flex justify-center items-center", selectedCurrencyType === item ? "bg-white-13 text-gallery" : "text-casper")} onClick={() => handleCurrencyTypeClick(item)}>{item}</div>)
          }
        </div>
            ...
            <>
              ...
              <div className="p-4 grid grid-cols-3 gap-4 rounded-[12px] bg-white-4">
                ...
                <div className="flex items-center flex-col border-l border-r border-white-13">
                  ...
                  <span className="text-[0.875rem] text-white font-bold">0.2</span>
                </div>
                <div className="flex items-center flex-col">
                  <span className="text-[10px] text-casper">Total win or loss</span>
                  <span className="text-[0.875rem] text-dodger-blue font-bold">-0.2</span>
                </div>
              ...
              <div className="rounded-t-[8px] overflow-hidden">
                <div className="grid gap-4 grid-cols-[auto_70px_70px_70px] p-4 bg-mirage  items-center text-casper font-bold text-[12px]">
                  <span>Game</span>
                  <span className="flex justify-center items-center">Number of Bets</span>
                  <span className="flex justify-center items-center">Bet amount</span>
                  <span className="flex justify-center items-center">Win or lose</span>
                </div>
                {
                  games.map((game, index) => <div key={index} className={cn("grid h-12 gap-4 grid-cols-[auto_70px_70px_70px] px-4 py-2 items-center font-bold text-[12px]", index % 2 === 0 ? "bg-white-4": "bg-white-8")}>
                    <div className="flex gap-2 items-center text-casper">
                      <img src={game.icon} alt="game" />
                      {game.name}
                    </div>
                    <div className="flex justify-center items-center text-white">{game.numberBet}</div>
                    <div className="flex justify-center items-center text-white">{game.betAmount}</div>
                    <div className="flex justify-center items-center text-dodger-blue">{game.win}</div>
                  </div>)
                }
              </div>
            </>
