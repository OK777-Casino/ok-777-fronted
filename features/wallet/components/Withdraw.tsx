    ...
    <div className=" [@media(max-width:660px)]:w-full">
      <div className="w-full flex flex-col gap-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 bg-white-4 rounded-[0.75rem] overflow-hidden p-1 h-11">
          {
            ["Crypto", "Fiat"].map((item, index) => <div className={cn("text-[0.875rem] font-bold rounded-[0.5rem] overflow-hidden flex justify-center items-center", selectedCurrencyType === item ? "bg-white-13 text-gallery" : "text-casper")} onClick={() => handleCurrencyTypeClick(item)}>{item}</div>)
          }
        </div>
          ...
          selectedCurrencyType === "Crypto" ? (
            <>
              ...
              <div className="p-4 flex gap-4 rounded-[0.75rem] bg-white-4 flex-col">
                  ...
                  <input value={0} className="px-4 h-[47px] flex items-center bg-white-8 rounded-[0.5rem] text-casper font-bold text-[0.75rem]" />
