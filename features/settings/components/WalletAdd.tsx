...
const WalletAdd: React.FC = () => {
  ...
  const tabs = [
    {
      id: "add", desc: "Add address",
    },
    {
      id: "activate", desc: "Activation address"
    }
  ]
  ...
  return (
    <div className=" [@media(max-width:660px)]:w-full">
      <div className="w-full  flex flex-col gap-4">
        {/* WalletAdd Header */}
        <h1 className="text-lg sm:text-xl hidden lg:block font-bold text-white ">Wallet Address</h1>
        ...
        <div className="p-4 flex flex-col gap-4 items-center text-casper">
          ...
          <p className="text-[0.875rem]">You have not added a wallet address yet.</p>
        </div>
        <TDButton onClick={toggleModal} type="blue" className="w-[14.8125rem] gap-3 mx-auto h-[2.5625rem] text-gallery text-[0.875rem] font-bold">
            <PlusIcon />
            Add wallet address
        </TDButton>
        <h2 className="uppercase indent-[1.25rem] text-white font-bold text-[1.125rem]">faqs</h2>
        <FAQ faqs={faqs} title={false} />
      </div>
      <ModalContainer isOpen={isModalOpen} onClose={toggleModal} title="Activation address" className="lg:w-[30%]">
          <div className="flex flex-col gap-2">
            <div className="p-1 rounded-[0.75rem] bg-white-4 gap-4 grid grid-cols-2">
              {
                tabs.map(tab => <div onClick={() => setSelectedTab(tab.id)} className={cn("h-9 font-bold text-[0.875rem] flex justify-center items-center rounded-[0.5rem]", tab.id === selectedTab ? "bg-white-13 text-white": "text-casper")}>{tab.desc}</div>)
              }
            </div>
            {
              selectedTab === "add" ? 
              <>
                <div className="p-4 rounded-[0.75rem] bg-white-4 flex flex-col gap-4">
                  <h2 className="text-[0.875rem] font-bold text-white">Your address</h2>
                  <input
                    type="text"
              ...
              <>
                <div className="p-4 rounded-[0.75rem] bg-white-4 flex flex-col gap-4">
                  <h2 className="text-[0.875rem] font-bold text-white flex justify-between items-center ">Transfer activation amount  <img src="/icons/coin-icon/USDT.svg" className="w-4 h-4"/></h2>
                  <CopyBox className="bg-white-8">0.0888</CopyBox>
                </div>
                <div className="p-4 rounded-[0.75rem] bg-white-4 flex flex-col gap-4">
                  <h2 className="text-[0.875rem] font-bold text-white">Activate transfer address</h2>
                  <CopyBox className="bg-white-8"><span className="text-dodger-blue">TXS3</span>PfAU9hemKkoBWRUfsUkGBSrZGa<span className="text-dodger-blue">gh6X</span></CopyBox>
                </div>
              </>
            }
            <h2 className="text-[0.875rem] font-bold text-white">Activated address: <span className="text-dodger-blue">0</span></h2>
                {
                  collapsedAddress ?
                  ...
                  <div className="grid font-bold text-[12px] bg-mirage  border-b border-white-13 grid-cols-[20%_40%_40%] items-center h-[56px] text-blue-bayoux">
                      ...
                      <span className="flex justify-center items-center">Address</span>
                  ...
                  <div className="grid font-bold text-[12px] bg-white-4   grid-cols-[20%_40%_40%] items-center h-[48px] text-white">
                      <span className="flex justify-center items-center">
                        <img src="/icons/coin-icon/USDT.svg" className="w-6 h-6 mx-auto" />
                      </span>
                      <span className="flex justify-center items-center">TGFf7****ePvThd</span>
                      <span className="flex justify-center items-center font-medium">To be activated</span>
                  </div>
                </div>:
                <div className="h-px bg-mirage-4 w-full" />
                }
                <div onClick={toggleAddress} className="mx-auto">
                  {collapsedAddress ? <ChevronsUpIcon  className="w-[26px] h-[26px] mx-auto" color="#A7B5CA" /> : <ChevronsDownIcon className="w-[26px] h-[26px]" color="#A7B5CA" />}
                </div>
          </div>
      </ModalContainer>
    </div>
