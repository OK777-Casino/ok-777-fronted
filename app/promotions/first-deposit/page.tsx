  ...
  return (
    <BetTemplate {...data} >
      <div className="p-4 bg-white-4 rounded-[12px] flex flex-col gap-4">
        <div className="text-[18px] font-bold text-white flex justify-center items-center">Activity Fund Application</div>
        <div className="pb-4 grid grid-cols-1 xl:grid-cols-3 2xl:grid-cols-6 justify-between items-center rounded-[12px]  gap-4">
          <TXCard title="Additional rewards" fee="188U" value="30" desc={<>Cumulative number of invitees</>} />
          <TXCard title="Additional rewards" fee="188U" value="100" desc={<>Cumulative number of invitees</>} />
          <TXCard title="Additional rewards" fee="188U" value="500" desc={<>Cumulative number of invitees</>} />
          <TXCard title="Additional rewards" fee="188U" value="1000" desc={<>Cumulative number of invitees</>} />
          <TXCard title="Additional rewards" fee="188U" value="3000" desc={<>Cumulative number of invitees</>} />
          <TXCard title="Additional rewards" fee="188U" value="5000" desc={<>Cumulative number of invitees</>} />
        </div>
        <div className="p-4 rounded-[12px] flex justify-between items-center bg-white-8 flex justify-center items-center text-[14px] text-casper">
          <div>Total number of invitees  <b className="text-dodger-blue">0</b></div>
          <div className="hidden md:block">Bonus  <b className="text-dodger-blue"> 0U</b></div>
        </div>
      </div>

      <div className="bg-white-4 rounded-[12px] p-4 flex flex-col gap-4 mt-4 ">
        <div className="font-bold text-[18px] flex justify-center items-center text-white">How to claim the bonus</div>
        <div className="grid xl:grid-cols-2 lg:grid-cols-1 gap-4">
          <div className="p-8 flex flex-col gap-4 bg-white-4 rounded-[12px]">
            <div>
              <TabButton type="one" title="Step One" className="h-[23px] text-[12px] font-bold text-white" />
              <span className="text-[14px] text-casper font-bold pt-4">Copy the invitation link and recommendation code to your friends</span>
            </div>
            <CopyBox className="bg-white-8">https://ok777.casino/?AgentCode=330395</CopyBox>
          ...
          <div className="p-8 flex flex-col gap-4  bg-white-4 rounded-[12px]">
            <div className="flex flex-col gap-4 items-start">
              <TabButton type="one" title="Step Two" className="h-[23px] text-[12px] font-bold text-white" />
              <span className="text-[14px] text-casper font-bold">(Assist) friends to complete registration and recharge</span>
            </div>
            <div className="relative flex justify-center items-center">
              <img src="/images/Device1.png" className="h-[224.03px] mx-auto" alt="phone" />
              <div className="w-[320px] py-[10px] z-[999] pr-[9px] pl-[16px] bg-mirage absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/3 flex justify-between items-center shadow-[0_8px_16px_#3389FF40] w-[312px] rounded-[16px]">
                      ...
                      <div className="flex gap-2 relative">
                        <BlackButton onClick={() => {}} className="text-[12px] text-casper">Log in</BlackButton>
                        <TDButton onClick={()=>{}} className="w-[85px] h-[33px] rounded-[8px] text-[12px]" type="red">Register</TDButton>
                        <img src="/icons/cursor.svg" alt="cursor" className="rotate absolute left-[15%]" />
                      </div>
                    </div>
            </div>
          </div>
        ...
        <div className="p-8 grid 2xl:grid-cols-[40%_60%] items-center grid-cols-1 gap-8 justify-between bg-white-4 rounded-[12px]">
          <div>
            <TabButton type="one" title="Step Three" className="h-[23px] text-[12px] font-bold text-white" />
            <span className="text-[14px] text-casper font-bold pt-4">Unlock additional rewards and claim them to your account</span>
          </div>
          <div className="relative flex justify-center items-center">
