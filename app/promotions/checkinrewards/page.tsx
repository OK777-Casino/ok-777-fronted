...
const CheckinRewardPage = () => {
  ...
  const rewards = [
    "1U", "2u", "5u", "5u", '5u', '5u', '24u + 18u'
  ]
  return (
    <BetTemplate {...data} >
      <div className="p-4 bg-white-4 rounded-[12px] flex flex-col gap-4">
        <div className="text-[18px] font-bold text-white flex justify-center items-center"><span>Activity Fund Application</span></div>
        <div className=" grid grid-cols-10 justify-between items-center rounded-[12px]  gap-4">
          {
            rewards.map((reward, index) => <DailyRewardCard className={index > 4 ? "col-span-5": "col-span-2"} reward={reward} key={index} date={index + 1} />)
          }
        </div>
      </div>

      <div className="p-4 bg-white-4 rounded-[12px] flex flex-col mt-4 gap-4">
        <div className="text-[14px]  text-casper flex justify-center items-center w-[70%] mx-auto"><span>Sign in The seventh day continuously, and you can unlock additional rewards. After signing out, it will be recalculated from the first day</span></div>
        <div className=" grid grid-cols-1  2xl:grid-cols-3 justify-between items-center rounded-[12px]  gap-4">
          <div className="p-4 bg-white-4 rounded-[12px] flex flex-col items-center gap-4">
            <span className="text-white text-[14px] font-bold"><span>0Day</span></span>
            <span className="text-dodger-blue text-[14px] font-bold"><span>Continuous sign-ins this round</span></span>
          </div>
          <div className="p-4 bg-white-4 rounded-[12px] flex flex-col items-center gap-4">
            <span className="text-white text-[14px] font-bold"><span>0U</span></span>
            <span className="text-dodger-blue text-[14px] font-bold"><span>Reward received</span></span>
          </div>
          <div className="p-4 bg-white-4 rounded-[12px] flex flex-col 2xl:flex-row items-center gap-4">
            ...
            <div className="flex flex-col gap-4 items-center">
              ...
              <span className="text-dodger-blue text-[14px] font-bold">Rewards to be collected this round</span>
            </div>
          </div>
