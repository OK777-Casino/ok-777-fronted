...
export default function RecentPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0E141C] text-white">
      ...
      <div className="max-w-7xl mx-auto p-4">
        ...
        {filteredGames.length > 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-400">
              {searchTerm ? 'Try adjusting your search terms' : 'Start playing games to see them here!'}
            </p>
          </div>
        )}
      </div>
      ...
      {(isProviderOpen || isTimeFilterOpen) && typeof window !== 'undefined' && createPortal(
        <div
        ...
        >
          ...
          {activeDropdown === 'timeFilter' && timeFilters.map((filter) => (
            <div
              key={filter.id}
              onClick={() => {
              ...
              className={`w-full px-4 py-3 text-left hover:bg-white-8 transition-colors cursor-pointer ${
                selectedTimeFilter.id === filter.id
                  ? "bg-blue-500/20"
                  : ""
              }`}
            >
